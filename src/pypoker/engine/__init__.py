...
from pypoker.constants import HandType, CardSuit
...
def find_consecutive_value_cards(
    cards: List[Card], treat_ace_low: bool = True, run_size: int = None
) -> List[List[Card]]:
    """
    ...
    cards_by_value = group_cards_by_value(cards)
    card_values = [
    ...
    runs = _find_consecutive_numbers(card_values)
    if run_size:
        runs = _split_consecutive_runs_to_size(runs, run_size)
...
def deduplicate_card_sets(card_sets: List[List[Card]]):
    """
    Public helper method
    """

    # Order cards in each card set
    card_sets = [order_cards(card_set) for card_set in card_sets]
